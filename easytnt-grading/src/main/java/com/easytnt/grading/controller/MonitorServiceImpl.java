...
public class MonitorServiceImpl implements MonitorService {
	...
	public List<IchartData> subjectsMonitor(Long testId) {
		...
		List<IchartData> datas = jdbcTemplate.query(sql1, new Object[] {testId}, new RowMapper<IchartData>() {

			@Override
			public IchartData mapRow(ResultSet rs, int rowNum) throws SQLException {
				IchartData data = new IchartData();
				data.addRow(rs.getFloat("paper_name"),rs.getFloat("total"),rs.getFloat("finish"));
				return data;
			}
	...
	@Override
	@Transactional(readOnly=true)
	public List<IchartData> sameTeamMonitor(Teacher teacher) {
		...
		List<IchartData> datas = jdbcTemplate.query(sql1, new Object[] {teacher.getTeacherAccount()}, new RowMapper<IchartData>() {

			@Override
			public IchartData mapRow(ResultSet rs, int rowNum) throws SQLException {
				IchartData data = new IchartData();
				data.addRow(rs.getFloat("teacher_name"),rs.getFloat("teacher_account"),
						rs.getInt("total"),rs.getFloat("_max"),rs.getFloat("_min"),rs.getFloat("_avg")
						,rs.getFloat("spenttime"));
				return data;
			}
	...
	@Override
	@Transactional(readOnly=true)
	public List<IchartData> teamMonitorOfWorking(Teacher teacher) {
	...
	@Override
	@Transactional(readOnly=true)
	public List<IchartData> teamMonitorOfStabled(Teacher teacher) {
		// TODO Auto-generated method stub
		return null;
	}
